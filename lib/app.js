...
const app = new Apper(
  ...
  },
  {
    ...
    syncFrequency: '15m',
    staticPaths: [path.resolve(`${__dirname}/../static`)],
    defaultFilter: {
...
app.onStop((ctx, signal) => {
  ctx.logger.info(`Received signal ${signal}, stopping calendar syncer polling...`);
  if (syncer) {
    syncer.stopPolling();
  }
});
...
app.get('/calendars', async (context, req, res) => {
  ...
  try {
    ...
    const gcal = syncer ? syncer.getGCalSyncer() : null;

    const result = await Promise.all(
      calendars.map(async (cal) => {
        const live = sourceMap.get(cal.name);
        let targetCalendarName = cal.targetCalendarName || null;
        if (gcal && cal.targetCalendarId && !targetCalendarName) {
          try {
            targetCalendarName = await gcal.getCalendarSummary(
