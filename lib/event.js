...
class Event {
  ...
  constructor(
    ...
    this.#isAllDay = Boolean(
      raw &&
        (raw.datetype === 'date' ||
          (raw.start && raw.start.dateOnly) ||
          raw.isAllDay)
    );
    ...
    if (
      ...
      raw &&
      raw.start &&
      typeof raw.start.getFullYear === 'function'
    ) {
      const sy = raw.start.getFullYear();
      const sm = String(raw.start.getMonth() + 1).padStart(2, '0');
      const sd = String(raw.start.getDate()).padStart(2, '0');
      this.#startDate = `${sy}-${sm}-${sd}`;

      if (raw.end && typeof raw.end.getFullYear === 'function') {
        const ey = raw.end.getFullYear();
        const em = String(raw.end.getMonth() + 1).padStart(2, '0');
        const ed = String(raw.end.getDate()).padStart(2, '0');
        this.#endDate = `${ey}-${em}-${ed}`;
      } else {
